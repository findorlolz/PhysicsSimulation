...
__forceinline static bool intersect_bb(const float orig[3], const float dirInv[3],
			const float bbMin[3], const float bbMax[3], float tmin)
{
	float tnear =  (bbMin[0] - orig[0])*dirInv[0];
	float tfar = (bbMax[0] - orig[0])*dirInv[0];
	if (tnear > tfar)
		std::swap(tnear, tfar);

	float tynear = (bbMin[1] - orig[1])*dirInv[1];
	float tyfar = (bbMax[1] - orig[1])*dirInv[1];
	if (tynear > tyfar)
		std::swap(tynear, tyfar);
	if (tnear > tyfar || tynear > tfar)
		return false;
	tnear = std::max(tnear, tynear);
	tfar = std::min(tfar, tyfar);

	float tznear = (bbMin[2] - orig[2])*dirInv[2];
	float tzfar = (bbMax[2] - orig[2])*dirInv[2];
	if (tznear > tzfar)
		std::swap(tznear, tzfar);
	if (tnear > tzfar || tznear > tfar)
		return false;
	tnear = std::max(tnear, tznear);
	tfar = std::min(tfar, tzfar);
